        ...
        match sync_info:
            case LastSyncInfo.SUCCESSFUL:
                if p not in [l.audio_file_path for l in self.config.lyrics_successful_sync]:
                    ...
                    self.config.lyrics_failed_sync = [
                        l for l in self.config.lyrics_failed_sync if l.audio_file_path != p
                    ]
            case LastSyncInfo.ALREADY_SYNCED:
                if p not in [l.audio_file_path for l in self.config.lyrics_already_synced]:
            ...
            case LastSyncInfo.FAILED:
                if p not in [l.audio_file_path for l in self.config.lyrics_failed_sync]:
                    self.config.lyrics_failed_sync.append(LyricsRecord(sync_level, p, time.time()))
